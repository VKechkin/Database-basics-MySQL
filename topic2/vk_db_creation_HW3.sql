...
CREATE TABLE messages (
	...
	from_user_id BIGINT UNSIGNED NOT NULL, -- внешние ключи
    to_user_id BIGINT UNSIGNED NOT NULL,   -- внешние ключи
    body TEXT,
...
DROP TABLE IF EXISTS `musics_albums`;
...
CREATE TABLE `musics` (
	...
	FOREIGN KEY (album_id) REFERENCES musics_albums(id) ON UPDATE CASCADE ON DELETE CASCADE,
    FOREIGN KEY (media_id) REFERENCES media(id) ON UPDATE CASCADE ON DELETE CASCADE
);
...
CREATE TABLE `games`(
      id SERIAL PRIMARY KEY,
      name VARCHAR(255),
      games_types_id BIGINT UNSIGNED NOT NULL, -- внешние ключи
      user_id BIGINT UNSIGNED NOT NULL,
    	body text, -- описание игры
      filename VARCHAR(255), -- ссылка на игру
     `size` INT, -- размер файла
	metadata JSON,         -- Набор пар ключ: значение 
      created_at DATETIME DEFAULT NOW(),
      updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
-- тоже самое что и NOW(),

      FOREIGN KEY (user_id) REFERENCES users(id) ON UPDATE CASCADE ON DELETE CASCADE,
      FOREIGN KEY (games_types_id) REFERENCES games_types(id) ON UPDATE CASCADE ON DELETE CASCADE
);
